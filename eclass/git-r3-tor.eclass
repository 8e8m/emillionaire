...
# @FUNCTION: _git-r3-tor_set_gitdir
# @USAGE: <repo-uri>
# @INTERNAL
# @DESCRIPTION:
...
# @FUNCTION: _git-r3-tor_set_submodules
...
# @FUNCTION: _git-r3-tor_set_subrepos
...
# @FUNCTION: _git-r3-tor_is_local_repo
# @USAGE: <repo-uri>
# @INTERNAL
# @DESCRIPTION:
# Determine whether the given URI specifies a local (on-disk)
# repository.
_git-r3-tor_is_local_repo() {
	debug-print-function ${FUNCNAME} "$@"
...
git-r3-tor_fetch() {
	...
	local -x GIT_DIR
	_git-r3-tor_set_gitdir "${repos[0]}"
	...
	for r in "${repos[@]}"; do
		if [[ ! ${EVCS_OFFLINE} ]]; then
			...
			local fetch_command=( git -c 'http.https://*.onion.proxy=socks5h://127.0.0.1:9050' fetch "${r}" )
			...
			if [[ ${clone_type} == shallow ]]; then
				if _git-r3-tor_is_local_repo; then
	...
	if git cat-file -e "${local_ref}":.gitmodules &>/dev/null; then
		local submodules
		_git-r3-tor_set_submodules "${_GIT_SUBMODULE_PATH}" \
		...
		while [[ ${submodules[@]} ]]; do
			...
			then
				...
				local subrepos
				_git-r3-tor_set_subrepos "${url}" "${repos[@]}"

				_GIT_SUBMODULE_PATH=${_GIT_SUBMODULE_PATH}${path}/ \
				git-r3-tor_fetch "${subrepos[*]}" "${commit}" \
					"${local_id}/${subname}" ""
			fi
...
# and therefore must be used consistently with git-r3-tor_fetch.
...
# for respective git-r3-tor_fetch.
#
...
git-r3-tor_checkout() {
	...
	if ! git cat-file -e refs/git-r3/"${local_id}"/__main__; then
		die "Logic error: no local clone of ${repos[0]}. git-r3-tor_fetch not used?"
	fi
	local remote_ref=$(
	...
	git-r3-tor_sub_checkout
	unset -f git-r3-tor_sub_checkout
	...
	if [[ -f ${out_dir}/.gitmodules && ! ${checkout_paths} ]]; then
		...
		while [[ ${submodules[@]} ]]; do
			...
			if [[ -d ${out_dir}/${path} ]]; then
				...
				_GIT_SUBMODULE_PATH=${_GIT_SUBMODULE_PATH}${path}/ \
				git-r3-tor_checkout "${subrepos[*]}" "${out_dir}/${path}" \
					"${local_id}/${subname}"
			fi
...
# @FUNCTION: git-r3-tor_peek_remote_ref
...
git-r3-tor_pkg_needrebuild() {
	debug-print-function ${FUNCNAME} "$@"

	local new_commit_id=$(git-r3-tor_peek_remote_ref)
...
pkg_needrebuild() { git-r3-tor_pkg_needrebuild; }
